...
$(function(){
    ...
    window.getBaseMessage = function(clearMessage, id, type, flag, detaileId, relateId) {
        ...
        $.get(UIPage.context.url + '/object/detailInformation/' + id + '/' + type, function(data) {
    ...
    window.baseBasic = function(data, id, type, pageType, icon, parentNodeId, objectType) {
        ...
        var image = pageType === "document" ? "" :
            //'<img id="topo_detail_photo" src="' + UIPage.context.url + '/image/' + icon + '/' + pageType + '"/>';
        ...
        $(".base_relate_list").on('mouseover', function() {
            ...
            setTimeFlag = setTimeout(function() {
                ...
                if ($(_this).attr('data-show') === "false") {
                    ...
                    $.get(UIPage.context.url + '/object/detailInformation/' + $(_this).attr("data-id") + '/' + $(_this).attr("data-type"), function(data) {
                        if (!data) {
                            return false;
                        }
    ...
    var mapHandler = function(e) {
        ...
        // console.log(dropObj);
        var topoObj = {};
        $.get(UIPage.context.url + "/object/" + "partInformation/" + dropObj.id + "/" + dropObj.type,function(searchNodes) {
    ...
    window.getTotalMessage = function(idArr) {
        ...
        $.ajax({
            type: "POST",
            url: UIPage.context.url + '/statistic',
            data: {
                "objectId": idArr.ids,
                "type": idArr.types
            },
    ...
    $("#make_ensure").on('click', function() {
        ...
        $.post(UIPage.context.url + '/document/createobject', makeObj, function(data) {
            if (!data) {
                return false;
            }
            var datas = JSON.parse(data);
    ...
    function getSearchData(value) {
        if (!!value) {
            ...
            $.get(UIPage.context.url + '/objects/' + value, function(data) {
                $("#topology_search_loading").hide();
