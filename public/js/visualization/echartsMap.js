      ...
      $.get('/js/public/mapdata/china.json', function(chinaJson) {
        ...
        function dblClickCommon(params, clickType){
          ...
          if(parseInt(mnParams.region.id)>1000 && parseInt(mnParams.region.id)<9900){//市范围 
            setMapLevel(3, mnParams, '/js/public/mapdata/geometryCouties/'+mnParams.region.id+'00.json',clickBank);
          }else if(parseInt(mnParams.region.id)>99){	//全国范围
            setMapLevel(1, mnParams, '','');
          }else{//第二层 省范围
            setMapLevel(2, mnParams, '/js/public/mapdata/geometryProvince/'+provience[0].properties.id+'.json', clickBank);
          }
        }
