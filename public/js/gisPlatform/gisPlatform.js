...
$(function(){
    ...
    window.bayonetName = UIPage.context.gis.bayonet; //存放context.json 里面默认的 卡口名称
    //调整pgis代码的问题！！
    if(UIPage.context.gis.type==="PGIS"){
        setTimeout(function () {
  ...
  $(".gisPlatform_button_sure").on('click', function () {
      ...
      if(noFill.length > 0){
        ...
        if(localStorage.getItem('dashboardId')){
          $.ajax({
            url: UIPage.context.url+'/dashboard/publish',
            type: 'POST',
            data: {
          ...
          $.ajax({
            url: UIPage.context.url+'/dashboard/publish/create',
            type: 'POST',
            data: paraData,
  ...
  function showGisTable(param){
    ...
    $.ajax({
      url: UIPage.context.url + '/warning/page/total',
      type: 'POST',
      ...
      success: function (data) {
        ...
        function mapTableCallback(index){
          $.ajax({
            url: UIPage.context.url + '/warning/page',//UIPage.context.url + '/object/getKeyAreaGisList'
            type: 'POST',
            data: {
              "keyword":param,
              "type":"person",
  ...
  function showBayonetTable(param){
    ...
    $.ajax({
      url: UIPage.context.url + '/gis/bayonet/page',//UIPage.context.url + '/object/getKeyAreaGisList'
      type: 'POST',
      ...
      success: function (data) {
        ...
        function mapTableCallback(index){
          $.ajax({
            url: UIPage.context.url + '/gis/bayonet',//UIPage.context.url + '/object/getKeyAreaGisList'
            type: 'POST',
            data: {
              "keyword":keyword,
              "pageNo":index,
  ...
  function showKeyAreaTable(idstr, param){
    ...
    let keyword = param ? param :"null";
    if(idstr === "no"){
      let ajaxurl = UIPage.context.url + '/gis/keyArea/search/page';
      let ajaxdata = { "keyword":keyword };
      let ajaxBackUrl = UIPage.context.url + '/gis/keyArea/search';
      $.ajax({
        url: ajaxurl,
        type: 'POST',
        data: ajaxdata,
      ...
      let ajaxurl = UIPage.context.url + '/gis/keyArea/searchbyId';
      let ajaxdata = { "areaIds":keyword };
      let ajaxBackUrl = UIPage.context.url + '/gis/keyArea/searchbyId';
      ...
      function mapKeyAreaIDCallback(index){
        ...
        $.ajax({
          url: UIPage.context.url + '/gis/keyArea/searchbyId',
          type: 'POST',
          data: {
            "areaIds": kw
          },
    ...
    function setKeyAreaResult(eventdata){
      ...
      $(".gis-keyarea-table-remove").unbind("click").bind("click",function () {
        ...
        const url = UIPage.context.url + '/object/deleteKeyArea';
      ...
      if(warningData.length > 0){
        ...
        if(mvSign && mvSign === "movepath"){//行动轨迹
          ...
          for(let w=0;w<warningData.length;w++){
            $.ajax({
              url: UIPage.context.url + '/object/path/gis',
              type: 'get',
              data: {
          ...
          if(warningData.length == 2){
            $.ajax({
              url: UIPage.context.url + '/object/path/mergeAnalysis',
              type: 'post',
              data: {
  ...
  function addKeyAreaToGis(cameraSign) {
    ...
    if(cameraSign){
      $.ajax({
        url: UIPage.context.url + '/gis/bayonet/id',
        type: 'post',
        data: {
          id: cameraArr
        },
  ...
  function addWarningToGis(pathSign) {
    ...
    $.ajax({
      url: UIPage.context.url + '/object/path/gis/multi',
      type: 'post',
      data: {
  ...
  function clearTimeLineInterval(){ //为了操作效果，只要动了时间轴，就禁止轨迹小车等的移动
      ...
      setTimeout(function () {
        ...
        if(nodedata.length > 0){//判断是否有数据
            ...
            if(selectGisById.length > 0){
                $.ajax({
                    url: UIPage.context.url + '/object/gis',
                    type: 'post',
                    data: {
                        id:selectGisById,
    ...
    function toggleGisStyle(data) {
        if(!mapload){
            if(UIPage.context.gis.type==="PGIS"){
            ...
            }else if(UIPage.context.gis.type==="bmap"){
                $("#map_area_statistics").css("display","block");
                $("#map_gis_table").css("display","block");

                Promise.all([mapLoadScriptPromise('/js/public/maps/gis/DrawingManager.js'),
                  mapLoadScriptPromise('/js/public/maps/gis/LuShu.js'),
                  mapLoadScriptPromise('/js/public/maps/gis/Heatmap.js'),
                  mapLoadScriptPromise('/js/public/maps/gis/TextIconOverlay.js'),
                  mapLoadScriptPromise('/js/public/maps/gis/CurveLine.js'),
                  mapLoadScriptPromise('/js/public/maps/gis/MarkerClusterer.js'),
            ...
            }else if(UIPage.context.gis.type==="arcgis"){
                $("#map_area_statistics").css("display","block");
                $("#map_gis_table").css("display","block");
                //加载css
                $("<link>").attr({ rel: "stylesheet",type: "text/css",href: UIPage.context.gis.cssFirst }).appendTo("head");

                if(UIPage.context.gis.cssSecond !== "null"){
                    $("<link>").attr({ rel: "stylesheet", type: "text/css", href: UIPage.context.gis.cssSecond }).appendTo("head");
                }
                ...
                scripttt.src = "/js/public/maps/arcGis/loadCluster.js";
                scripttt.type = 'text/javascript';
                scripttt.id = 'loadScriptGis';
                headtt.appendChild(scripttt);

              Promise.all([mapLoadScriptPromise('/js/public/maps/arcGis/mapJson.js'),
                mapLoadScriptPromise('/js/gisPlatform/arcGisNgm.js'),
                mapLoadScriptPromise(UIPage.context.gis.js)
              ]).then(function () {
            ...
            }else if(UIPage.context.gis.type==="supermap"){
                /*
                 * topology.jsx中加
        ...
        this.svg = d3.select(".topoMenu")[0][0] === 'g' ? d3.select(".topoMenu") : d3.select("g.main"),
            ...
            this.getMoreMenus = function() {
                ...
                for (var i = 0; i < type.length; i++) {
                    (function(i) {
                        const url = UIPage.context.url + '/nextType/';
    ...
    mapWorkMenus.prototype.oneTopoMenus = function(config,markORarea) {
                ...
                switch (d.value) {
                    ...
                    case 1:
                        ...
                        if (d.data === 'path') {
                            ...
                            if(UIPage.context.gis.type==="arcgis"){
                              mapPathBasePoint = mapCommon.mapWorkMarker[0].graphic.attributes;
                            }

                            if(mapPathBasePoint.type === UIPage.context.gis.bayonet){
        ...
        selectedPath.append("image")
            ...
            /*.attr("xlink:href", function(d, i) {
                return UIPage.context.url + "/static/typeicon/" + d.data.toLowerCase() + ".svg";
            });*/
    };
    ...
    window.mapCommonPart = {
        ...
        mapFilter:function(){
            ...
            if (propertyArr.length>0) {
                ...
                $.post( UIPage.context.url + '/search/filter', datas, function( data ) {
        ...
        menu:function(conf,type){
          ...
          if(type === "keyAreamenu"){
            ...
            if(UIPage.context.gis.type==="bmap"){
        ...
        mapMainRadraw:function (marker, thr, id, type, nodeId, objtype, extendUrl){
            let ajaxurl = UIPage.context.url + extendUrl;
            let completed = function (){ return false; };
            let judgment = function() { return false; };
            let succeed = function(data){
                var graph = data;
        ...
        getmapGis:function (parseDatas,selectGisById,selectGisByType,marker,multiSign){
            let ajaxurl = UIPage.context.url + '/object/gis';
        ...
        setPassCar: function (){
          ...
          if(startTime && endTime){
            ...
            return '';*/
              ...
              let ajaxurlPage = UIPage.context.url + '/object/gis/event';
          ...
          function mapKKXXPageCallback(index){
            ...
            let ajaxurl = UIPage.context.url + '/object/gis/event';
  ...
  gisAllData.prototype.getSearchModulePage = function (param) {
    ...
    return new Promise(function(resolve, reject) {
      _thatGisData.getAjaxBase(UIPage.context.url + '/object/page',param,'POST').then(function (data) {
        resolve(data);
      }).catch(function () {
        reject("未查询到数据!");
      });
  ...
  gisAllData.prototype.getSearchModuleData = function (param) {
    let _thatGisData = this;
    return new Promise(function(resolve, reject) {
      _thatGisData.getAjaxBase(UIPage.context.url + '/search',param,'POST').then(function (data) {
        resolve(data);
      }).catch(function () {
        reject("未查询到数据!");
      });
  ...
  gisAllData.prototype.getSearchModuleDragData = function (dropObj) {
    let _thatGisData = this;
    return new Promise(function(resolve, reject) {
      _thatGisData.getAjaxBase(UIPage.context.url + "/object/partInformation/" + dropObj.id + "/" + dropObj.type,null,'GET').then(function (data) {
        resolve(data);
      }).catch(function () {
        reject("未查询到数据!");
      });
  ...
  gisAllData.prototype.getObjectGis = function (param) {
    let _thatGisData = this;
    return new Promise(function(resolve, reject) {
      _thatGisData.getAjaxBase(UIPage.context.url + '/object/gis',param,'POST').then(function (data) {
        resolve(data);
      }).catch(function () {
        reject("未查询到数据!");
      });
    ...
    gisAllDatas.getSearchModuleData(mapSearchKeyWord).then(function (data) {
      ...
      for (let i = 0; i < dataSource.length; i++) {
        ...
        otherHTML += '<div class="mapResultPanel" draggable="true" data-name="'+propty[0].value+'" data-pageType="'+dataSource[i].page_type+'" data-objectType="entity" data-id="'+ dataSource[i].id+'" data-nodeId="'+dataSource[i].nodeId+'" data-type="'+dataSource[i].objectType+'"><div class="mapResultPanelIn">' +
          ...
          /*'<img src="'+ UIPage.context.url + '/image/'+ dataSource[i].icon + '/entity" alt="头像"/>' +*/
  ...
  searchModule.prototype.dragDomToMap = function (className) {
    ...
    if(UIPage.context.gis.type === "bmap"){
      ...
      targetMap = $('#basemap').get(0);
    }else if(UIPage.context.gis.type === "PGIS"){
      targetMap = $('#basemap').get(0);
    }
